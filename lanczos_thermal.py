...
# folder = f'/home/amoschen08/ED/{time_now}/'
folder = f'./{time_now}/'
...
def lanczos_method(A, B, H, E0, k_d, N_samples, w, beta):
    ...
    for ni in range(N_samples):
        ...
        Id_FT = np.squeeze(c[0, ...])
        
        # save results to a list
        O_FT_list.append(results_FT)
        ...
        del E1, V1, lv1, E2, V2, lv2, results_FT, Id_FT
        gc.collect()

    return O_FT_list, Z_FT_list
...
# temperature
T_L = np.array([0.01, 0.05, 0.1, 0.2, 0.3, 0.5, 0.7, 1])
...
o1t, z1t = lanczos_method(Sx1, Sx2, H, E0, k_d, N_samples, w, beta_L)
o2t, z2t = lanczos_method(Sy1, Sy2, H, E0, k_d, N_samples, w, beta_L)  
o3t, z3t = lanczos_method(Sz1, Sz2, H, E0, k_d, N_samples, w, beta_L)
...
for i in range(len(w)):
    ...
    for j in range(len(T_L)):
        ...
        o1, do1 = bootstrap_mean((o1t[:, i, j] + o2t[:, i, j]), (z1t[:, j] + z2t[:, j]))
        o2, do2 = bootstrap_mean(o3t[:, i, j], z3t[:, j])
...
plt.plot(w[:], do1_FT[:, 2].real, label='Sxy_r error')
plt.plot(w[:], do1_FT[:, 2].imag, label='Sxy_i error')

plt.plot(w[:], do2_FT[:, 2].real, label='Szz_r error')
plt.plot(w[:], do2_FT[:, 2].imag, label='Szz_i error')

#plt.plot(w[1:Nw], -(Szz_e[1:Nw, 0]/Z_e[0]).real, label='Szz_r')
#plt.plot(w[1:Nw], -(Szz_e[1:Nw, 0]/Z_e[0]).imag, label='Szz_i')


plt.legend(loc='best')

plt.savefig(folder + '2D_{0:d}{1:d}_Saa_T={2:.1f},Q=({3:.2f},{4:.2f})pi,dmi={5:.2f},error.png'.format(n, m, T_L[2], 2*n1/n, 2*n2/m, J_dmi))

plt.clf()

# plt.plot(w[:], o1_FT[:, -1].real, label='Sxy_r')
# plt.plot(w[:], o1_FT[:, -1].imag, label='Sxy_i')

# plt.plot(w[:], o2_FT[:, -1].real, label='Szz_r')
# plt.plot(w[:], o2_FT[:, -1].imag, label='Szz_i')

# #plt.plot(w[1:Nw], -(Szz_e[1:Nw, -1]/Z_e[-1]).real, label='Szz_r')
# #plt.plot(w[1:Nw], -(Szz_e[1:Nw, -1]/Z_e[-1]).imag, label='Szz_i')

# plt.legend(loc='best')
# plt.savefig(folder + '2D_{0:d}{1:d}_Saa_T={2:.1f},Q=({3:.2f},{4:.2f})pi,dmi={5:.2f}.png'.format(n, m, T_L[-1], 2*n1/n, 2*n2/m, J_dmi))
...
if H.Ns < 20000:
    ...
    np.savez(folder + "DSF_{0:d}{1:d}_Q_{2:.2f}{3:.2f},dmi={4:.2f}.npz".format(n, m, 2*n1/n, 2*n2/m, J_dmi), Sxy=o1_FT, Szz=o2_FT, Szz_e=Szz_e, Z=Z_e, T=T_L, w=w, E=E_e, V=V_e)

else:

    np.savez(folder + "DSF_{0:d}{1:d}_Q_{2:.2f}{3:.2f},dmi={4:.2f}.npz".format(n, m, 2*n1/n, 2*n2/m, J_dmi), Sxy=o1_FT, Szz=o2_FT, T=T_L, w=w)

E_e, V_e, Szz_e, Z_e = dsf_exact(Sz1, Sz2, H, w, beta_L)

# np.savez(folder + "DSF_{0:d}{1:d}_Q_{2:.2f}{3:.2f},dmi={4:.2f}.npz".format(n, m, 2*n1/n, 2*n2/m, J_dmi),
#         Z=Z_e, E=E_e, V=V_e)

for i, t in enumerate(T_L):
    L_result = pd.DataFrame(
        {
            'w': w,
            'Sxy': o1_FT.T[i, :],
            'Szz': o2_FT.T[i, :],
            'Szz_e': Szz_e.T[i, :],
            'sigma_Sxy': do1_FT.T[i, :],
            'sigma_Szz': do2_FT.T[i, :]
        }
    )
    L_result.to_csv(folder+'DSF_{0:d}{1:d}_Q_{2:.2f}{3:.2f},dmi={4:.2f},T={5:.2f}.csv'.format(n, m, 2*n1/n, 2*n2/m, J_dmi, t),
        index=False
        )
    del L_result


print(f'N_sample={N_samples}, basis={k_d}' + ", simulation took {0:.4f} sec".format(time()-ti))
print(f'Jxy={Jxy}, Jz={Jz}, J_nnn={J_nnn}, J_dmi={J_dmi}, h={h}.')
