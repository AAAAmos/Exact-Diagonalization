...
T_L = [0.01, 0.05, 0.1, 0.2, 0.3, 0.5, 0.7, 1]
T_L = np.array(T_L)
...
plt.plot(w[:], o1_FT[:, 2].real, label='Sxy_r')
plt.plot(w[:], o1_FT[:, 2].imag, label='Sxy_i')

plt.plot(w[:], o2_FT[:, 2].real, label='Szz_r')
plt.plot(w[:], o2_FT[:, 2].imag, label='Szz_i')
...
if H.Ns < 20000:
  ...
  np.savez("./data/DSF_{0:d}{1:d}_Q_{2:.2f}{3:.2f},dmi={4:.2f}.npz".format(n, m, 2*n1/n, 2*n2/m, J_dmi), Sxy=o1_FT, Szz=o2_FT, Szz_e=Szz_e, Z=Z_e, T=T_L, w=w, E=E_e, V=V_e)

else:

  np.savez("./data/DSF_{0:d}{1:d}_Q_{2:.2f}{3:.2f},dmi={4:.2f}.npz".format(n, m, 2*n1/n, 2*n2/m, J_dmi), Sxy=o1_FT, Szz=o2_FT, T=T_L, w=w)
